...
export interface Transaction {
  ...
  amount: number;
  upiId?: string;      // keep optional for compatibility
  upiLink?: string;    // optional if some code supplies it
...
export default function PaymentQRModal({
  ...
  // Build UPI URI from available data (prefer transaction.upiLink if provided)
  const payUri =
    transaction.upiLink ??
    `upi://pay?pa=${encodeURIComponent(
      transaction.upiId ?? transaction.toUser.upiId
    )}&pn=${encodeURIComponent(transaction.toUser.name)}&am=${transaction.amount}&cu=INR`;
  ...
  return createPortal(
    <div className="fixed inset-0 z-[999999] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="relative bg-gradient-to-br from-slate-900/95 via-slate-800/90 to-slate-900/95 backdrop-blur-xl border border-white/10 rounded-3xl shadow-2xl p-8 max-w-md w-full overflow-hidden">
        ...
        <button onClick={onClose} className="absolute top-4 right-4 p-2 rounded-xl bg-white/10 hover:bg-white/20">
          <X className="w-5 h-5" />
        </button>
        ...
        <div className="relative z-10 flex flex-col items-center space-y-6">
          ...
          {/* QR uses payUri built from upiId */}
          ...
          <div className="w-full space-y-3">
            <button onClick={handleCopyUPI} className="w-full p-3 rounded-xl bg-gradient-to-r from-slate-700 to-slate-800 text-white">
              <Copy className="w-4 h-4 inline-block mr-2" />
              Copy UPI ID
            </button>

            {transaction.status === "pending" ? (
              <button onClick={onConfirmPayment} disabled={loading} className="w-full p-3 rounded-xl bg-green-500 text-white">
